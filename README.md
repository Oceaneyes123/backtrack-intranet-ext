...
- **Fast failure handling** — unsent draft bubbles are removed immediately if delivery fails
...
`chrome.storage.sync` has a strict per-item and total quota. Writes are debounced and should never happen per keystroke or per message.
